<div class="p-7 z-50">
    ...
    <div class="flex flex-col gap-3 mb-5">
        <h1 class="text-gray-700">Do you wish to remove section <span class="font-semibold">{{ $section->sec_Section }}</span>?</h1>

        <p>Note: This will remove {{ App\Models\Student::where('sec_id', $section->sec_id)->count() }} students from the database.</p>
    </div>

    <form action="{{ route('sections.destroy', $section) }}" method="POST" class="flex flex-row gap-2 justify-end">
        @csrf
        @method('DELETE')
