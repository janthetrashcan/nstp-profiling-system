<x-dashboard-layout>
    ...
    <div class="flex flex-row pr-4 mb-6 justify-between items-center">
        ...
        <div id="functions-lhs" class="flex flex-row gap-x-3">
            <a href="{{ route('dashboard.formatorlist') }}" class="bg-gray-200 hover:bg-gray-300 transition-all duration-200 flex flex-row w-fit h-12 px-4 py-2 justify-start items-center rounded-lg gap-2 shadow-md">
                <x-carbon-arrow-left class="h-6" />
                <span class="font-semibold">Return</span>
            </a>
            <a href="{{ route('dashboard.formatoredit', $formator->f_id) }}" class="bg-blue-500 hover:bg-blue-600 text-white transition-all duration-200 flex flex-row w-fit h-12 px-4 py-2 justify-start items-center rounded-lg gap-2 shadow-md">
                <x-carbon-edit class="h-6" />
                <span class="font-semibold">Edit</span>
            </a>
            <form action="{{ route('formator.destroy', $formator->f_id) }}" method="POST" id="deleteSingleForm" onclick="return confirm('Are you sure?')">
                @csrf
                @method('DELETE')
    ...
    <div id="formator-profile" class="flex flex-col md:flex-row gap-6 justify-center">
        ...
        <div class="flex flex-col gap-6">
            <!-- Program Information -->
            <div id="formator-program" class="flex flex-col gap-y-4 p-6 w-full md:w-96 rounded-xl bg-white shadow-lg">
